...
import { Search, AlertTriangle, CheckCircle, Info, XCircle, Tag, Download, ChevronRight, ChevronDown } from 'lucide-react';
    ...
    const csvContent = [
      ...
      ...filtered.map(l => {
        return [
          ...
          l.status,
          l.latency,
          `"${l.fullRequest.replace(/"/g, '""')}"`, // escape quotes
    ...
    <div className="flex flex-col h-[600px] bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-700 rounded-xl overflow-hidden shadow-xl transition-colors duration-300">
      ...
      <div className="flex-1 overflow-auto bg-slate-50 dark:bg-[#0a0f1e] p-2 font-mono text-xs md:text-sm">
        {filtered.slice(0, 500).map((log, i) => {
          ...
          return (
            <div key={log.id} className={`group flex flex-col hover:bg-slate-200 dark:hover:bg-slate-800/30 p-1 rounded transition-colors border-l-2 ${isExpanded ? 'border-blue-400 dark:border-blue-500 bg-slate-100 dark:bg-slate-800/50' : 'border-transparent hover:border-slate-400 dark:hover:border-slate-600'}`}>
              <div 
              ...
              >
                ...
                <div className="break-all text-slate-700 dark:text-slate-300 flex-1">
                   ...
                   {log.logType !== 'HTTP' && (
                       <span className={`px-1.5 rounded text-[10px] mr-2 ${
                           log.isError ? 'bg-red-100 dark:bg-red-500/20 text-red-700 dark:text-red-300' : 'bg-emerald-100 dark:bg-emerald-500/20 text-emerald-700 dark:text-emerald-300'
                       }`}>
                ...
                <div className="mt-2 ml-7 pl-6 py-2 border-l border-slate-300 dark:border-slate-700">
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-4">
                    <div className="space-y-1">
                      ...
                      <div className="text-xs text-slate-700 dark:text-slate-300"><span className="text-slate-500">Method:</span> {log.method}</div>
                      <div className="text-xs text-slate-700 dark:text-slate-300"><span className="text-slate-500">Path:</span> {log.path}</div>
                      <div className="text-xs text-slate-700 dark:text-slate-300"><span className="text-slate-500">Status:</span> {log.logType === 'HTTP' ? log.status : (log.isError ? 'ERROR' : 'SUCCESS')}</div>
                      <div className="text-xs text-slate-700 dark:text-slate-300"><span className="text-slate-500">Latency:</span> {log.latency} ms</div>
                      <div className="text-xs text-slate-700 dark:text-slate-300"><span className="text-slate-500">Bytes:</span> {log.bodyBytes} B</div>
                    </div>
                    <div className="space-y-1">
                      <div className="text-[10px] text-slate-400 uppercase font-bold tracking-wider">Client Info</div>
                      <div className="text-xs text-slate-700 dark:text-slate-300 break-all"><span className="text-slate-500">IP:</span> {log.remoteIp}</div>
                      <div className="text-xs text-slate-700 dark:text-slate-300 break-all"><span className="text-slate-500">UserAgent:</span> {log.userAgent}</div>
                      <div className="text-xs text-slate-700 dark:text-slate-300 break-all"><span className="text-slate-500">Referer:</span> {log.referer}</div>
                    </div>
                  </div>
                    ...
                    <div className="mb-4">
                      ...
                      <div className="flex gap-2 flex-wrap text-slate-700 dark:text-slate-300">
                        {Object.entries(log.metadata).map(([k, v]) => {
                          ...
                          return (
                            <span key={k} className="flex items-center gap-1 text-[10px] bg-slate-200 dark:bg-slate-800/80 px-2 py-0.5 rounded border border-slate-300 dark:border-slate-700">
